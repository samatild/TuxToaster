...
def io_test(directory, temp_file_path, operation_type):
    if operation_type == "read":
        dd_command = f"while true; do dd if={temp_file_path} of=/dev/null bs=4096K iflag=direct; done"
    else:
        dd_command = f"while true; do dd if=/dev/zero of={temp_file_path} bs=4096K oflag=direct; done"
