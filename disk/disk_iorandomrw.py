...
def io_test(directory, temp_file_path):
    while not exit_signal.is_set():
        ...
        if operation_type == "read":
            dd_command = f"dd if={temp_file_path} of=/dev/null bs=4K iflag=direct count=100"
        else:
            dd_command = f"dd if=/dev/zero of={temp_file_path} bs=4K oflag=direct count=100"
        
        dd_process = subprocess.Popen(dd_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setsid)
        dd_process.wait()
        
        # Check if the process needs to be terminated
