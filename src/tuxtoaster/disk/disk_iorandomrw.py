...
def io_test(directory, temp_file_path):
    while not exit_signal.is_set():
        ...
        if operation_type == "read":
            dd_command = f"dd if={temp_file_path} of=/dev/null bs=4K iflag=direct count=100 status=none"
        else:
            dd_command = f"dd if=/dev/zero of={temp_file_path} bs=4K oflag=direct count=100 status=none"
...
def run_iorandom():
    ...
    print(f"{BOLD}{GREEN}Running DISK IOPS - RANDOM RW — press ENTER to stop.{RESET}")
